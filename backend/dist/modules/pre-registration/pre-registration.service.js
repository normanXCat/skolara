...
class PreRegistrationService {
    ...
    async create(data) {
        ...
        if (existingRecord) {
            ...
            error.details = [
                {
                    path: ["childFirstName"],
                    message: "Une pré-inscription existe déjà pour cet enfant pour cette année scolaire.",
                },
            ];
            throw error;
        }
    ...
    async generateFileNumber() {
        ...
        const lastRecord = await this.repository.findFirst({ fileNumber: { startsWith: prefix } }, { fileNumber: "desc" });
        let sequence = 1;
