...
export class AdminGradesRepository {
    /**
    ...
    async getStats() {
        const [totalGrades, avgResult, distribution] = await Promise.all([
            prisma.grade.count(),
            prisma.grade.aggregate({ _avg: { value: true }, _max: { value: true }, _min: { value: true } }),
            prisma.grade.findMany({ select: { value: true } }),
        ]);

        const values = distribution.map((g: { value: number }) => g.value);
