...
export function FooterClient({ settings }: FooterClientProps) {
    ...
    // Build dynamic socials — only show those with a URL
    const socials = [
        { icon: <IconBrandFacebook className="size-5" />, href: settings.facebook_url, label: "Facebook" },
        { icon: <IconBrandInstagram className="size-5" />, href: settings.instagram_url, label: "Instagram" },
        { icon: <IconBrandLinkedin className="size-5" />, href: settings.linkedin_url, label: "LinkedIn" },
        { icon: <IconBrandX className="size-5" />, href: settings.twitter_url, label: "X" },
    ].filter((s) => s.href && s.href.trim() !== "");

    const displayEmail = settings.email || "contact@skolara.com";
    const displayPhone = settings.phone || "+212 5 00 00 00 00";
    const displayLocation = (settings.city || settings.address)
